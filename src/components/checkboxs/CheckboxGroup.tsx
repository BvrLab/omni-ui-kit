...
const CheckboxGroup = ({
    ...
    const [optionsProp, setOptionsProps] = useState<CheckboxGroupOption[]>(options);
    ...
    return (
        ...
        >
            <fieldset>
                {legendName && (<legend className="sr-only">{legendName}</legend>)}
                <Flexbox direction='column' gap='md'>
                    <div className="space-y-5">
                        {optionsProp.map((option) => renderOption(option, handleCheckboxChange, isDisabled, colorScheme, textSize))}
                    </div>
                </Flexbox>
            </fieldset>
        </div>
